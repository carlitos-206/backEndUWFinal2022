...
describe("/fires routes", () => {
  ...
  describe("GET /fires/in/:month/:year", () => {
   ...
   // test get all fires by incident name
   describe("GET /fires/name/:name", () => {
    it("should return a fire's data on success", async () => {
      mongoData.getFireByIncidentName.mockResolvedValue({
        _id: "62fb42131c5b7ea309f7e0e0",
        total_acres: null,
        containment_datetime: "2014-08-24T18:59:59Z",
        control_datetime: "2014-08-24T19:01:00Z",
      ...
      const res = await request(server).get("/fires/name/JOHNSON%20CANYON");
      expect(res.statusCode).toEqual(200);
      expect(res.body.error).not.toBeDefined();
    });
    it("should return a status code of 404 on failure", async () => {
      mongoData.getFireByIncidentName.mockResolvedValue(null);
      const res = await request(server).get("/fires/name/JOHNSON%20CAN");
      expect(res.statusCode).toEqual(404);
      expect(res.body.error).toBeDefined();
    });
  });
